...
do
    ...
    echo ${logName}
    bash scripts/spawn/spawn.sh -n 3 -t 6 -b -j -c -l -e 30 -r -s ${syncSchemes[$i]} -d 25 -g ${logName}
    until [ -f "$fullLogName" ]
    do
...
do
    ...
    fullLogName="${logName}log.csv"
    echo ${logName}
    bash scripts/spawn/spawn.sh -n 3 -t 6 -b -j -c -l -e -10 -r -s ${syncSchemes[$i]} -d 25 -g ${logName}
    until [ -f "$fullLogName" ]
    do
    echo "Current test is still running.."
    sleep 30
