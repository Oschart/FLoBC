...
do
    ...
    trainer_assigned_port=$(($((start_public_port))+$(($((i))%$((node_count))))))
    echo $start_public_port
    trainer_noise=$(echo "$i * $accumulated_error_scale" | bc)
    rm $lightclient/ModelMetadata
    openTab $command_start "npm start --prefix $lightclient -- $trainer_assigned_port models/test_model/data.csv $trainer_noise"
    sleep 10
done
